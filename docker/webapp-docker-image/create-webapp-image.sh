...
# 获取应用程序
rm -rf ROOT.war
wget http://172.16.2.42/job/diteng-beta/ws/diteng-app/target/diteng-app-1.0.0-SNAPSHOT.war -O ROOT.war

# 构建应用镜像
docker build -t $SOURCE_IMAGE:$TAG .

# 标记镜像版本
docker tag $SOURCE_IMAGE:$TAG $REGISTRY_HOST/$NAMESPACE/$SOURCE_IMAGE:$TAG
